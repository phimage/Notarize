    ...
    func run(_ options: NotarizeCommand.Options) -> Result<(), NotarizeCommand.ClientError> {
        ...
        do {
            if let info = try process.run(on: appURL) {
                let output = "\(info)"
                //let output = String(data: try ? JSONEncoder().encode(info) ?? Data())
                //let output = String(data: try ? PropertyListEncoder().encode(info) ?? Data())
                print(output)
            }
            return .success(())
        } catch {
...
struct NotarizeOptions: OptionsProtocol {
    ...
    static func create(_ path: String?) -> (_ username: String?) -> (_ password: String?) -> (_ ascProvider: String?) -> (_ config: String?) -> NotarizeOptions {
        return { username in
            return { password in
                return { ascProvider in
                    return { config in
                        self.init(path: path, username: username, password: password, ascProvider: ascProvider, configurationFile: config)
                    }
                }
            }
        }
